#!/usr/bin/env python3
"""Test dual probe detection and divergence measurement."""
...
def test_dual_detection():
    """Test both activation and text probes on same token."""

    print("\n" + "=" * 80)
    print("DUAL PROBE DETECTION & DIVERGENCE TEST")
    print("=" * 80)

    # Load manager
    print("\nLoading probe manager...")
    manager = DynamicProbeManager(
        probes_dir=Path('results/adaptive_test_tiny'),
        base_layers=[0],
    ...
    print(f"\nLoaded:")
    print(f"  Activation probes: {len(manager.loaded_activation_probes)}")
    print(f"  Text probes: {len(manager.loaded_text_probes)}")
    ...
    for prompt in test_cases:
        ...
        for token_idx, token_text in enumerate(tokens):
            ...
            for concept_key, text_probe in manager.loaded_text_probes.items():
                try:
                    prob = text_probe.pipeline.predict_proba([token_text])[0, 1]  # Prob of class 1
